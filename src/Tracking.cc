...
namespace ORB_SLAM2
{
    Tracking::Tracking(System *pSys,                 // SLAM 系统对象
                       ORBVocabulary* pVoc,          // 字典对象
                       FrameDrawer *pFrameDrawer,    // 帧渲染器
                       MapDrawer *pMapDrawer,        // 地图渲染器
                       Map *pMap,                    // 地图对象
                       KeyFrameDatabase* pKFDB,      // 关键帧数据库对象
                       const string &strSettingPath, // 配置文件路径
                       const int sensor)             // 相机类型 MONOCULAR=0, STEREO=1, RGBD=2
    ...
    cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft,  // 校畸变之后的左目图像
                                      const cv::Mat &imRectRight, // 校畸变之后的右目图像
    ...
    // 1. 若 imRGB 是彩色图，则转换为灰度图
    // 2. 根据配置参数 mDepthMapFactor 调整深度图的单位
    // 3. 构建 Frame 对象
    // 4. 调用 Track() 完成实际的轨迹跟踪
    cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, // 彩色图或灰度图
    ...
    // 1. 若未完成初始化，则进行初始化
    // 2. 估计相机位姿，各种形式的 Track
    // 3. 根据局部地图优化相机位姿，Track Local Map
    // 4. 判定是否需要生成新的关键帧，则生成之
    void Tracking::Track()
    {
